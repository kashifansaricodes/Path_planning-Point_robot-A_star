...
while True:
    ...
    if (start_pixel_value == [255, 0, 0]).all() or (start_pixel_value == [0, 0, 255]).all() or (5 >= start_x >= (map.shape[0]-5)) or (5 >= start_y < (map.shape[1]-5)):
...
while True:
    ...
    if (goal_pixel_value == [255, 0, 0]).all() or (goal_pixel_value == [0, 0, 255]).all() or (5 >= goal_x >= (map.shape[0]-5)) or (5 >= goal_x < (map.shape[1]-5)):
        ...
        cv2.circle(map, (goal_x, goal_y), 5, (255, 255, 255), -1)  # Draw a green circle at the goal
        break 
